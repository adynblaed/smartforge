...
def test_incident_and_rca_flow(internal_client):
    ...
    iid = inc.json()["id"]
    rca = internal_client.post(
        f"/api/v1/incidents/{iid}/rca",
        json={
            "root_cause": "Sensor failure",
            "corrective_actions": "Replace sensor",
        },
    )
...
def test_askai_ingest_and_list_documents(internal_client):
    ...
    r = internal_client.post(
        "/api/v1/ask-ai/documents",
        json={
            "title": "Test SOP",
            "kind": "sop",
            "content": "Lubricate weekly.",
        },
    )
    assert r.status_code == 200
...
def test_internal_askai_ask(internal_client):
    r = internal_client.post(
        "/api/v1/ask-ai/ask", json={"question": "How do I fix high vibration?"}
    )
    assert r.status_code == 200
...
def test_forge_ai_at_risk_highlights_a_machine(internal_client):
    r = internal_client.post(
        "/api/v1/ask-ai/forge", json={"question": "Which machine is most at risk?"}
    )
    assert r.status_code == 200
    body = r.json()
    assert body["answer"]
...
def test_forge_ai_locates_by_type(internal_client):
    r = internal_client.post(
        "/api/v1/ask-ai/forge", json={"question": "show me the press"}
    )
    assert r.status_code == 200
...
def test_knowledge_base_update_and_delete_404(internal_client):
    ...
    assert (
        internal_client.delete(f"/api/v1/ask-ai/knowledge-bases/{missing}").status_code
        == 404
    )
...
def test_knowledge_base_crud_and_askai_inclusion(internal_client):
    ...
    # the KB is surfaced as a source in AskAI answers (Forge Facts branding)
    ans = internal_client.post(
        "/api/v1/ask-ai/ask", json={"question": "What torque for press bolts?"}
    )
    assert ans.status_code == 200
    assert any(s["kind"] == "forge_fact" for s in ans.json()["sources"])

    # update
    upd = internal_client.patch(
        f"/api/v1/ask-ai/knowledge-bases/{kb_id}", json={"content": "Updated: 90 Nm."}
    )
    assert upd.status_code == 200 and "90 Nm" in upd.json()["content"]

    # delete
    assert (
        internal_client.delete(f"/api/v1/ask-ai/knowledge-bases/{kb_id}").status_code
        == 200
    )
