...
def seed_sandbox(session: Session) -> None:
    ...
    machines = [
        Machine(code="cnc-01", name="CNC Mill 01", machine_type=MachineType.cnc_mill,
                manufacturer="Haas", model="VF-2", factory_id=factory.id,
                line_id=line.id, pos_x=-4.0, pos_z=0.0, runtime_hours=1850),
        Machine(code="arm-01", name="Robotic Arm 01", machine_type=MachineType.robotic_arm,
                manufacturer="KUKA", model="KR-10", factory_id=factory.id,
                line_id=line.id, pos_x=0.0, pos_z=0.0, runtime_hours=1200),
        Machine(code="press-01", name="Hydraulic Press 01",
                machine_type=MachineType.hydraulic_press, manufacturer="Schuler",
                model="HP-400", factory_id=factory.id, line_id=line.id,
                pos_x=4.0, pos_z=0.0, runtime_hours=2100),
    ]
    ...
    for email, role in [
        ...
        if not crud.get_user_by_email(session=session, email=email):
            crud.create_user(session=session, user_create=UserCreate(
                email=email, password=settings.SANDBOX_USER_PASSWORD,
    ...
    for email, cust in [("buyer@acme-robotics.com", acme), ("buyer@globex-mfg.com", globex)]:
        if not crud.get_user_by_email(session=session, email=email):
            u = crud.create_user(session=session, user_create=UserCreate(
                email=email, password=settings.SANDBOX_USER_PASSWORD,
                full_name="Portal User", role=UserRole.customer))
    ...
    for i in range(6):
        ...
        session.add(CustomerOrder(
            customer_id=cust.id, order_number=f"SO-10{i}",
            part_type=_RNG.choice(["bracket", "housing", "gear", "flange"]),
            quantity=_RNG.randint(50, 500),
            estimated_completion=get_datetime_utc() + timedelta(days=_RNG.randint(2, 20)),
    ...
    session.refresh(s2)
    session.add_all([
        InventoryItem(sku="STL-4140", name="Steel 4140 bar", material_type="steel",
                      quantity=120, reorder_threshold=100, supplier_id=s1.id,
                      factory_id=factory.id),
        InventoryItem(sku="AL-6061", name="Aluminum 6061 block", material_type="aluminum",
                      quantity=40, reorder_threshold=80, supplier_id=s2.id,
                      factory_id=factory.id),
        InventoryItem(sku="HYD-OIL", name="Hydraulic oil", material_type="consumable",
                      quantity=15, reorder_threshold=25, supplier_id=s1.id,
                      factory_id=factory.id),
    ])

    # Jobs
    session.add_all([
        Job(customer="Acme Robotics", part_type="bracket", quantity=200,
            priority=2, status=JobStatus.approved, factory_id=factory.id),
        Job(customer="Globex Manufacturing", part_type="housing", quantity=350,
            priority=1, status=JobStatus.scheduled, factory_id=factory.id),
    ])

    # Knowledge documents (RAG corpus)
    session.add_all([
        KnowledgeDocument(
            title="CNC Mill Vibration Troubleshooting", kind="troubleshooting",
            machine_id=machines[0].id, tags="vibration,bearing,spindle",
        ...
        KnowledgeDocument(
            title="Hydraulic Press Overheating SOP", kind="sop",
            machine_id=machines[2].id, tags="temperature,coolant,hydraulic",
        ...
        KnowledgeDocument(
            title="Robotic Arm Fault Codes", kind="manual",
            machine_id=machines[1].id, tags="fault,error,controls",
        ...
        KnowledgeDocument(
            title="Preventive Maintenance Standard", kind="sop", tags="maintenance,pm",
    ...
    for shift in ["A", "B"]:
        for m in machines:
            run = ProductionRun(
                line_id=line.id, machine_id=m.id, shift=shift,
                planned_units=1000, actual_units=_RNG.randint(820, 980),
                scrap_units=_RNG.randint(5, 40), rework_units=_RNG.randint(2, 20),
    ...
    for m in machines:
        session.add(MachineConfiguration(
            machine_id=m.id, version=1, is_current=True, is_recommended=False,
            approved=True, speed=1200, temperature=60, pressure=120,
            feed_rate=0.25, tooling_profile="standard", material_type="steel"))
        session.add(MachineConfiguration(
            machine_id=m.id, version=2, is_current=False, is_recommended=True,
            approved=False, performance_delta=_RNG.uniform(3, 9),
            speed=1350, temperature=58, pressure=128, feed_rate=0.28,
            tooling_profile="optimized", material_type="steel"))
    session.commit()

    # Closed-loop recommendations (2D)
    recs = [
        Recommendation(machine_id=machines[0].id, line_id=line.id, category="config",
                       title="Increase spindle speed to 1350 RPM",
                       detail="Historical runs show +6% throughput with stable vibration.",
                       confidence=0.78),
        Recommendation(machine_id=machines[2].id, line_id=line.id, category="maintenance",
                       title="Advance hydraulic oil change by 1 week",
                       detail="Temperature trend predicts overheating within 120 runtime hours.",
                       confidence=0.82, status=RecommendationStatus.accepted,
                       outcome_impact=4.5),
        Recommendation(machine_id=machines[1].id, line_id=line.id, category="config",
                       title="Reduce payload to cut cycle time variance",
                       detail="Rejected — payload required for current job mix.",
                       confidence=0.55, status=RecommendationStatus.rejected),
    ]
    ...
    incident = Incident(
        ...
        delayed_orders=1, downtime_minutes=95, estimated_cost=12500.0,
        severity=Severity.high, resolved=False)
    ...
    for i in range(14):
        session.add(PurchaseOrder(
            ...
            status=[PurchaseOrderStatus.open, PurchaseOrderStatus.received,
                    PurchaseOrderStatus.draft][i % 3],
    ...
    for i in range(24):
        ...
        insp = Inspection(part_id=part_id, line_id=line.id, defect_detected=detected,
                          defect_type=dtype, confidence=conf,
        ...
        if detected:
            session.add(Defect(
                inspection_id=insp.id, line_id=line.id, defect_type=dtype,
                part_id=part_id, scrap_cost=42.0, is_scrap=True))
    session.commit()


def _sop(session: Session, *, code: str, title: str, category: str,
         entity_type: str, machine_id, summary: str,
         sections: list[tuple[str, str, str]]) -> Sop:
    """Create an SOP with ordered (anchor, title, body) chapters."""
    sop = Sop(code=code, title=title, category=category, entity_type=entity_type,
              machine_id=machine_id, summary=summary)
    session.add(sop)
    session.commit()
    session.refresh(sop)
    for i, (anchor, sec_title, body) in enumerate(sections):
        session.add(SopSection(sop_id=sop.id, anchor=anchor, order_index=i,
                               title=sec_title, body=body))
    session.commit()
    return sop
...
def seed_tickets_and_sops(session: Session) -> None:
    ...
    parts_inv = {
        "BRG-SPN-01": InventoryItem(sku="BRG-SPN-01", name="CNC spindle bearing",
            material_type="spare-part", quantity=1, unit="ea", reorder_threshold=2,
            supplier_id=sid_steel, factory_id=fid),
        "SEAL-HYD-01": InventoryItem(sku="SEAL-HYD-01", name="Hydraulic seal kit",
            material_type="spare-part", quantity=4, unit="kit", reorder_threshold=2,
            supplier_id=sid_alloy, factory_id=fid),
        "ENC-JNT-01": InventoryItem(sku="ENC-JNT-01", name="Robotic arm joint encoder",
            material_type="spare-part", quantity=0, unit="ea", reorder_threshold=1,
            supplier_id=sid_alloy, factory_id=fid),
        "FLT-AIR-01": InventoryItem(sku="FLT-AIR-01", name="Air intake filter",
            material_type="consumable", quantity=12, unit="ea", reorder_threshold=6,
            supplier_id=sid_steel, factory_id=fid),
        "LUB-WAY-01": InventoryItem(sku="LUB-WAY-01", name="Way lubricant (1L)",
            material_type="consumable", quantity=8, unit="bottle", reorder_threshold=4,
            supplier_id=sid_steel, factory_id=fid),
    }
    ...
    sop_cnc = _sop(
        session, code="SOP-CNC-001", title="CNC Mill VF-2 — Operating & Maintenance SOP",
        category="maintenance", entity_type="machine",
        ...
        sections=[
            ...
            ("operation", "3. Normal Operation Limits",
            ...
            ("spindle-bearing-service", "4. Spindle Bearing Service",
            ...
            ("tool-balancing", "5. Tool Balancing",
    ...
    sop_press = _sop(
        session, code="SOP-PRESS-001", title="Hydraulic Press HP-400 — Operating & Maintenance SOP",
        category="maintenance", entity_type="machine",
        ...
        sections=[
            ...
            ("hydraulic-oil-service", "3. Hydraulic Oil & Seal Service",
            ...
            ("cooler-cleaning", "4. Cooler Cleaning",
            ...
            ("pressure-check", "5. Pressure Verification",
    ...
    sop_arm = _sop(
        session, code="SOP-ARM-001", title="Robotic Arm KR-10 — Operating & Troubleshooting SOP",
        category="troubleshooting", entity_type="machine",
    ...
    def ticket(code, title, machine, severity, status, window, sop, anchor,
               what, exec_, op, remediation, incident_id=None,
               acknowledged=None) -> MaintenanceTicket:
        t = MaintenanceTicket(
            code=code, title=title,
            machine_id=machine.id if machine else None,
            incident_id=incident_id, severity=severity, status=status,
            what_happened=what, executive_summary=exec_, operator_detail=op,
            remediation=remediation,
            sop_id=sop.id if sop else None, sop_anchor=anchor,
    ...
    def add_parts(t, items: list[tuple[InventoryItem | None, str, float]]) -> None:
        for inv, name, qty in items:
            session.add(MaintenanceTicketPart(
                ticket_id=t.id, inventory_item_id=inv.id if inv else None,
                name=name, qty_needed=qty))
        session.commit()

    def add_log(t, kind, message, author=None, tz=None) -> None:
        session.add(MaintenanceTicketLog(
            ticket_id=t.id, kind=kind, author_email=author, message=message, tz=tz))
        session.commit()

    # T1 — CNC vibration
    t1 = ticket(
        "TICKET-0001", "CNC-01 elevated vibration", cnc, "high", "open", 2,
        sop_cnc, "spindle-bearing-service",
    ...
    add_parts(t1, [(parts_inv["BRG-SPN-01"], "CNC spindle bearing", 2),
                   (parts_inv["LUB-WAY-01"], "Way lubricant (1L)", 1)])
    add_log(t1, "system", "Ticket opened automatically from vibration rule "
                          "(index 0.71 > 0.60).")

    # T2 — Press overheating (tied to the seeded incident, acknowledged)
    t2 = ticket(
        "TICKET-0002", "PRESS-01 hydraulic overheating", press, "critical",
        "acknowledged", 1, sop_press, "hydraulic-oil-service",
    ...
    add_parts(t2, [(parts_inv["SEAL-HYD-01"], "Hydraulic seal kit", 1),
                   (hyd_oil, "Hydraulic oil", 2)])
    add_log(t2, "system", "Ticket opened from temperature rule (92°C > 85°C); linked "
                          "to incident.")
    add_log(t2, "acknowledgement", "Acknowledged by maintenance@smartforge.com",
            author="maintenance@smartforge.com", tz="America/Detroit")
    add_log(t2, "note", "Need to order more of the hydraulic seal kit and oil before "
    ...
    t3 = ticket(
        "TICKET-0003", "ARM-01 repeated E101 encoder fault", arm, "medium",
        "in_progress", 3, sop_arm, "encoder-service",
    ...
    add_log(t3, "status_change", "Status changed open → in_progress",
            author="maintenance@smartforge.com")

    # T4 — CNC PM overdue
    t4 = ticket(
        "TICKET-0004", "CNC-01 preventive maintenance overdue", cnc, "low",
        "open", 7, sop_pm, "lubrication",
        ...
        remediation="Lubricate ways, replace air intake filter, inspect belts. "
                    "See @SOP-PM-001.")
    add_parts(t4, [(parts_inv["FLT-AIR-01"], "Air intake filter", 2),
