...
async def list_services(session: SessionDep, _user: InternalUser) -> Any:
    ...
    data = [
        {"name": "PostgreSQL", "category": "Datastore", "status": db_status,
         "detail": f"Primary operational database{ms(db_ms)}", "configurable": False},
        {"name": "Redis", "category": "Cache & Pub/Sub", "status": redis_status,
         "detail": f"{s.REDIS_HOST}:{s.REDIS_PORT}{ms(redis_ms)}",
         "configurable": True, "log_service": "redis"},
        {"name": "Qdrant Vector DB", "category": "RAG",
         "status": ("running" if qdrant_up else ("disabled" if not s.RAG_ENABLED else "offline")),
         "detail": s.QDRANT_URL, "configurable": True, "log_service": "qdrant"},
        {"name": "Anthropic Claude", "category": "AI / LLM",
         "status": "connected" if s.askai_enabled else "offline",
         "detail": s.ANTHROPIC_MODEL, "configurable": True},
        {"name": "Prometheus", "category": "Observability", "status": "running",
         "detail": "Metrics scrape target", "configurable": False},
        {"name": "Grafana", "category": "Observability", "status": "running",
         "detail": "Dashboards", "configurable": False},
        {"name": "Telemetry Simulator", "category": "Worker / Process",
         "status": sim_status, "detail": sim_detail,
         "configurable": True, "log_service": "worker"},
        {"name": "Fiix CMMS", "category": "Integration",
         "status": "connected" if fiix_live else "mock",
         "detail": s.FIIX_BASE_URL, "configurable": True},
        {"name": "ERP Sync", "category": "Integration", "status": "mock",
         "detail": "Bidirectional ERP adapter", "configurable": True},
        {"name": "MES Sync", "category": "Integration", "status": "mock",
         "detail": "Bidirectional MES adapter", "configurable": True},
        {"name": "Email / SMTP", "category": "Integration",
         "status": "configured" if s.emails_enabled else "disabled",
         "detail": s.SMTP_HOST or "not configured", "configurable": True},
    ]
