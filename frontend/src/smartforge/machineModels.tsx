...
export function CncMill({ running }: { running: boolean }) {
  ...
  return (
    <group>
      <RoundedBox args={[1.7, 0.7, 1.3]} radius={0.08} smoothness={4} position={[0, 0.35, 0]} castShadow receiveShadow>
        <meshStandardMaterial color={STEEL_LIGHT} metalness={0.85} roughness={0.28} envMapIntensity={1.2} />
      </RoundedBox>
      <RoundedBox args={[0.36, 1.25, 0.36]} radius={0.05} smoothness={4} position={[-0.55, 1.2, -0.32]} castShadow>
      ...
      <mesh ref={spindle} position={[0.1, 1.05, 0]} castShadow>
        ...
        <meshStandardMaterial color={CHROME} metalness={1} roughness={0.12} envMapIntensity={1.5} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group>
      ...
      <group ref={arm} position={[0, 0.5, 0]}>
        <RoundedBox args={[0.26, 1.05, 0.26]} radius={0.06} smoothness={4} position={[0, 0.5, 0]} castShadow>
          <meshStandardMaterial color="#ea7a09" metalness={0.6} roughness={0.35} />
        </RoundedBox>
        <RoundedBox args={[1.05, 0.22, 0.22]} radius={0.06} smoothness={4} position={[0.5, 1, 0]} rotation={[0, 0, 0.6]} castShadow>
...
export function Press({ running }: { running: boolean }) {
  ...
  useFrame((s) => {
    if (running && ram.current)
      ram.current.position.y = 1.15 + Math.abs(Math.sin(s.clock.elapsedTime * 3)) * 0.3
  })
  return (
    <group>
      <RoundedBox args={[1.5, 0.7, 1.5]} radius={0.08} smoothness={4} position={[0, 0.35, 0]} castShadow receiveShadow>
        <meshStandardMaterial color={STEEL_LIGHT} metalness={0.85} roughness={0.28} />
      </RoundedBox>
      {[-0.62, 0.62].map((x) => (
        <RoundedBox key={x} args={[0.22, 2, 0.22]} radius={0.05} smoothness={3} position={[x, 1.45, 0]} castShadow>
          <meshStandardMaterial color={STEEL} metalness={0.9} roughness={0.25} />
        </RoundedBox>
      ))}
      <RoundedBox ref={ram} args={[1.25, 0.32, 1.25]} radius={0.06} smoothness={4} position={[0, 1.15, 0]} castShadow>
        <meshStandardMaterial color={CHROME} metalness={1} roughness={0.16} envMapIntensity={1.4} />
      </RoundedBox>
    </group>
  )
}
    ...
    <group>
      ...
      >
        <meshStandardMaterial
          ...
          roughness={0.45}
        />
        <Edges scale={1.005} threshold={15} color={active ? color : "#2a2f3a"} />
      </RoundedBox>
